...
async def post(
    ...
    try:
        atleta_out = AtletaOut(id=uuid4(), created_at=datetime.utc_now(), **atleta_in.model_dump())
...
async def query(id: UUID4, db_session: DatabaseDependency) -> AtletaOut:
    ...
    if not atleta:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Não foi possível encontrar o id")

    return atleta

@router.patch(
    "/",
    summary="Editar dados de um atleta Atletas",
    status_code=status.HTTP_200_OK,
    status_code=status.HTTP_200_OK,
    response_model=AtletaOut,
)
